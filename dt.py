...
def boundaries_viewer(max_depth_values, data_fun):
    ...
    Parameters
    ----------
    max_depth_values : a list containing the values @ which we want to prune the tree.
    ...
    for value in max_depth_values:
        ...
        plot_boundary("{}_dt_{}".format(data_fun.__name__, str(value)), clf, X_train, y_train, mesh_step_size=0.1, title="Boundaries of the tree, from {}, with depth {}".format(data_fun.__name__, str(value)), inline_plotting=True)


def get_acc(value, data_fun):
    X_train, y_train, X_test, y_test = data_fun()
    clf = DecisionTreeClassifier(max_depth = value)
    clf = clf.fit(X_train, y_train)
